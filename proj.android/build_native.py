...
def do_build(cocos_root, ndk_root, app_android_root,ndk_build_param,sdk_root,android_platform,build_mode):
    ...
    build_mode_param = 'NDK_DEBUG=1'
    if build_mode == 'release':
        build_mode_param = 'NDK_DEBUG=0 NDK_APPLICATION_MK=jni/Application_release.mk'
	
    if ndk_build_param == None:
        command = '%s -j%d -C %s %s %s' % (ndk_path, num_of_cpu, app_android_root, ndk_module_path, build_mode_param)
    else:
        command = '%s -j%d -C %s %s %s %s' % (ndk_path, num_of_cpu, app_android_root, ''.join(str(e) for e in ndk_build_param), ndk_module_path, build_mode_param)
